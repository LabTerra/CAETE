...
main_path = '/home/luana/Documents/outputs_JUSTRUNNED_CAETE_just_bia_and_precision/lu'
...
spin_files = glob.glob(f"{main_path}/{grd_name}/spin[0-9][0-9].pkz")
num_spins = len(spin_files)
print(num_spins)
...
# Get area
with open(f"{main_path}/{grd_name}/spin35.pkz", 'rb') as fh:
    file = joblib.load(fh)
    area = file['area']
...
CT1 = pd.read_csv("code_table_allom.csv")
...
for run_name in run_names:
    ...
    folder_path = f"./csv/{run_name}_spin35"
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)

    folder_path2 = f"{main_path}/{run_name}/{grd_name}/csv"
