...
!=================================================================
   ...
   subroutine photosynthesis_rate(c_atm, temp,p0,ipar,sla_allom,c4,nbio,pbio,&
        & cl1_prod,cs1_prod,leaf_turnover,f1ab,vm, amax,max_height,height1)
      ...
      real(r_8),intent(in) :: leaf_turnover
      real(r_8),intent(in) :: cl1_prod
      real(r_8),intent(in) :: cs1_prod
      ! O
      ...
      vm = vcmax_a(nbio2, pbio2, sla_allom)      ! 10**vm_nutri * 1D-6
      ...
      index_leaf = leaf_area_index(cl1_prod, sla_allom)
      ...
      do n = num_layer,1,-1
         ...
         &(1.0D0-exp(-0.5D0*layer(n)%mean_lai))
      enddo
      !=======================================================
      ...
      do n = num_layer, 1, -1
         ...
         layer(n)%lused = layer(n)%linc * (1.0D0 - exp(-0.5D0 * layer(n)%mean_lai))
...
! 
  ...
  !===================================================================
   ...
   subroutine pls_allometry (dt,awood,height,diameter,&
      &crown_area)
      ...
      ! ALLOMETRIC CONSTANTS DERIVED FROM LPJ-GUESS OR LPJ-GUESS-FIRE
      !k_allom1 = 250.0D0
      !k_allom2 = 20.0D0
      !k_allom3 = 0.8D0
      !pi = 3.1415926536D0
      ...
      do p = 1, npft   !to grasses
         if(awood(p) .le. 0.0D0) then
            ...
            diameter(p) = (4.0D0 * (cawood(p) * 1.0D3) / (dwood(p) * 1.0D6) * 3.1415926536D0 * 20.0D0)&
            &**(1.0D0 / (2.0D0 + 0.8D0))
            height(p) = 20.0D0 * (diameter(p) ** 0.8D0)
            crown_area(p) = min(crown_area_max(p), 250.0D0 * (diameter(p) ** 1.6D0))
         endif
      enddo
