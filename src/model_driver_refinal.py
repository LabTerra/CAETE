...
def apply_spin(grid:grd)->grd:
    ...
    w, ll, cwd, rl, lnc = grid.bdg_spinup(start_date = "19790101", end_date = "19830101")
...
def get_xy_from_zone(zone):
    ...
    #     # x0, x1 = 235, 236
    #     # folder = "central"
    return (y0, y1, x0, x1, folder)

def get_stime(clim_metadata):
...
if __name__ == "__main__":
    ...
    y0, y1, x0, x1, folder = get_xy_from_zone(zone)
    ...
    if sombrero:
        ...
        if climatology == 1:
            clim_metadata = Path(os.path.join(s_data, model_root, "ISIMIP_HISTORICAL_METADATA.pbz2"))
            input_path = model_root
            stime = get_stime(clim_metadata)
            # # open co2 data
            ...
            clim_metadata = Path(os.path.join(clim_and_soil_data, f"{outf}-historical_METADATA.pbz2"))
            stime = get_stime(clim_metadata)
