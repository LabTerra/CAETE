...
need to generate inputs from a data source other than the bundled
...
| Serialization       | `pickle` (`_pickle as pkl`); default protocol used by the producer     |
| Producer in repo    | `bz2.BZ2File(fpath, mode='w'); pkl.dump(obj, fh)`                      |
| Consumer in repo    | `bz2.BZ2File(fpath, mode='r'); obj = pkl.load(fh)`                     |
| File extension      | `.pbz2`                                                                |
...
folder. Other workflows in the repo use prefixed variants
...
### 3.3 Reading example (illustrative)

```python
import bz2, _pickle as pkl, copy
...
to the consumer, but the producer in this repo emits them in this order:
...
- Identical length, equal to `len(time_dict["time_index"])` of the metadata
  ...
  cells.
- `dtype`: floating-point. `float32` is what the producer writes; `float64`
  also works. No masked arrays — fill any missing values before saving.
- Memory layout: contiguous; the producer uses `.copy(order="F")` for
  legacy reasons but C-contiguous arrays work equally well.
...
| Key    | Long name                | Required input unit          | Used in model as                                         | Producer source unit (ISIMIP) |
| ------ | ------------------------ | ---------------------------- | -------------------------------------------------------- | ----------------------------- |
...
gridcell (the producer extracts a single `(y, x)` element from a global
`(360, 720)` `.npy` array).
...
### 4.6 Reading example (illustrative)
...
## 5. Consistency requirements across the bundle
...
The block below is the smallest possible producer. It is for documentation
...
Before shipping a generated bundle, verify:
...
- [ ] Files are loadable on a CAETÊ-compatible Python environment
      (NumPy ≥ the producer's; pickle protocol ≤ that supported by the
      consumer's interpreter).

---

## 8. Running `pre_processing.py`

The bundled producer [`input/pre_processing.py`](pre_processing.py) reads
...
The producer **does not standardize time**. It does not convert calendars,
...
| Cadence               | Must already be daily, contiguous, in order. The producer does not check this; the model assumes daily. |
...
| Duplicates / gaps     | Not detected. The producer trusts the source.                               |
...
- **Units.** The producer copies the source values verbatim into the
...
- Producer: [`input/pre_processing.py`](pre_processing.py)
- Reference cells: [`input/central/`](central/), `cax/input_data_183-257.pbz2`,
  `k34/input_data_185-240.pbz2`
- Consumer init (full grid): `grd.init_caete_dyn` in [`src/caete.py`](../src/caete.py)
- Consumer init (single plot from in-memory dict): `plot.init_plot` in
